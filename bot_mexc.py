...
async def plan_scanner_loop():
    """EMA/ATR планировщик — запускается через 5 мин после закрытия 4h свечи."""
    global _last_plan_4h_block
    while True:
        try:
            now = datetime.now(timezone.utc)
            block = (now.hour // 4) * 4          # 0, 4, 8, 12, 16, 20
            minutes_since = (now.hour - block) * 60 + now.minute

            if minutes_since >= 5 and _last_plan_4h_block != block:
                _last_plan_4h_block = block
                print(f"[PlanScanner] 4h block {block:02d}:00 — starting...")
                try:
                    loop = asyncio.get_event_loop()
